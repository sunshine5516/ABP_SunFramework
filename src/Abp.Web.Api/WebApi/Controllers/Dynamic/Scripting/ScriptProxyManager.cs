    ...
    public class ScriptProxyManager: ISingletonDependency
    {
        ...
        public string GetScript(string name, ProxyScriptType type)
        {
            ...
            lock(CachedScripts)
            {
                ...
                {
                    var dynamicController = _dynamicApiControllerManager.GetAll().
            ...
            lock (CachedScripts)
            {
                ...
                if (!CachedScripts.ContainsKey(cacheKey))
                {
                    ...
                    foreach(var dynamicController in dynamicControllers)
                    {
                        ...
                        script.Append(proxyGenerator.Generate());
                        script.AppendLine();
                    }
        ...
        private static IScriptProxyGenerator CreateProxyGenerator
            (ProxyScriptType type, DynamicApiControllerInfo controllerInfo, bool amdModule)
        {
            switch(type)
            {
